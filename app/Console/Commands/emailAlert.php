...
class emailAlert extends Command
{
            ...
            foreach($sites as $site)
            {
                ...
                if($fs)
                {
                    ...
                    if($dbs->date_down != null)
                    {
                        ...
                        $data["status"] = true;
                        if($dbs->server_up_email !=null)
                        {
                            /**
                             * QUER DIZER QUE JA FOI LHE ENVIADO UM EMAIL
                             */
                            $insert =DB::table('user_server_dbs')->where("id",$site->id)->update($data);
                        }else
                        {
                            /**
                             * QUER DIZER QUE AINDA NÃO FOI LHE ENVIADO UM EMAIL
                             */
                            $data["server_up_email"] = true;
                            $insert =DB::table('user_server_dbs')->where("id",$site->id)->update($data);
    ...
    {
        $sitedbs = DB::table("down_up_servers")->where("site_id",$site)->first();
        if($verification)
        {
            ...
            if($sitedbs)
            {
                $verSite = DB::table("down_up_servers")->where("site_id",$site)
                ->update($data);
            }else
            {
